...
public class FormulaireConnexionController {
    ...
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            boolean idTropcourt = champMotDePasse.getText().length() < 8;
            boolean lettresOk = champMotDePasse.getText().matches(".*[a-zA-Z].*");
            boolean chiffresOk = champMotDePasse.getText().matches(".*[0-9].*");
            boolean majusculeOk = champMotDePasse.getText().matches(".*[A-Z].*");
            return idTropcourt || (!chiffresOk) || (!lettresOk) || !(majusculeOk);
          }
          ;
        };
  ...
  private void annuler() {
    ...
    champIdentifiant.textProperty().set("");
    champMotDePasse.textProperty().set("");
    labelMessage.textProperty().set("");
  }
}
